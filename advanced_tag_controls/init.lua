...
M.update = function(s) M.previous_tags = awful.tag.selectedlist(s) end
...
M.move_client_to_previous_tags = function(client, toggle_instead_of_move)
  ...
  local tag_or_tags = M.previous_tags[1]

  -- if there is no tag, do nothing
  if not tag_or_tags then return end
...
M.close_active_clients_on_tags = function(tags)
  ...
  for _, tag in pairs(tags) do
    for _, client in pairs(tag:clients()) do
      if not clients[client] then clients[client] = client end
    end
  end
  ...
  for _, c in pairs(clients) do
    ...
    for _, client_tag in pairs(c:tags()) do
      ...
      if not client_tag_is_selected then goto continue end
    end
