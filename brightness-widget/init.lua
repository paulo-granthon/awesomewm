...
local function worker(user_args)
  ...
  local brightness_file = '/sys/class/backlight/amdgpu_bl1/brightness'
  get_brightness_cmd = 'cat ' .. brightness_file
  set_brightness_cmd = brightness_file .. ' -S %d' -- <level>
  inc_brightness_cmd = brightness_file .. ' -A ' .. step
  dec_brightness_cmd = brightness_file .. ' -U ' .. step
  ...
  local update_widget = function(widget, stdout, _, _, _)
    ...
    if not brightness_level then return end
  ...
  function brightness_widget:set(value)
    ...
    spawn.easy_async(string.format(set_brightness_cmd, value), function()
      spawn.easy_async(get_brightness_cmd, function(out) update_widget(brightness_widget.widget, out) end)
    end)
  end
